...
#
# 【放寬過一輪,結論是不要放】定位改用模板比對後(失敗率 60%→3%)重新掃過門檻,
# 因為原本的擋下率高達 48%,看起來很有放寬空間。實測 182 張:
#     (250,650) 2.0  全對 40%   誤按 9 張   ← 維持
#     (120,1100) 4.0 全對 43%   誤按 23 張
#     (80,1500)  6.0 全對 46%   誤按 32 張,且負樣本開始失守(2/2 → 1/2)
# 換 +6 張判對要付 +14 張誤按,不划算 —— 兩種失敗的代價不對等:
#   誤按 = 真的按下錯方向鍵,白燒一次符文冷卻,還要等 RETRY_WAIT 才能重試
#   不輸出 = 退回 2 線 claude 判讀(約 4.8s),符文照樣解得開
# 所以這個門檻要往「寧可不報」的方向偏,不是往命中率偏。
AREA_OK = (250, 650)
AREA_RATIO_MAX = 2.0
