...
# 實測落點是離散的(目標 4/6 都停在 4,目標 8/10 都停在 8,顆粒度約 4),所以不必精確導航,
# 只要人已經在環內就整段跳過 —— 省掉走路時間,謎題窗也更寬裕。
RADIUS_MIN, RADIUS_MAX = 5, 7          # 內圈原本是 4,但實測落點剛好 4 時角色就蓋住紫標
                                       # (邊界值直接踩雷)→ 收到 5,留一格安全邊際。
                                       # 外圈原本是 8,實測 8 格有時採集不到 → 收到 7;
                                       # 落點超出 7 就會觸發補正走位往裡靠。
# 虛擬平台的半寬(見 _push_virtual_platform)。刻意跟 RADIUS_MAX 分開 ——
# 那是「按得到符文」的距離,這是「走得到」的落腳範圍,縮小採集半徑不該連帶縮小可走範圍。
VP_HALF_WIDTH = 10
APPROACH_DX = 6                        # 需要導航時的目標:落在環中間
