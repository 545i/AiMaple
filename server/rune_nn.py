...
【CNN 嘗試過,沒過驗收,預設關閉】原本想取代的是【色度分割】。實測:膠囊定位零
誤差、模板判向在乾淨輸入上 97.5%,而色度分割整體只有 41% —— 差距全部來自
_chroma_map/_seg 在箭頭被怪物、技能特效、地形疊到時把背景一起吃進去,分割是手寫
幾何規則最不擅長的那一段。但 CNN 實測整體正確率比色度分割更低(見下方表格),沒
過驗收門檻,`read_dirs` 預設仍全部走色度分割。
