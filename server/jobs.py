...
DEFAULT_MOVE = {
    ...
    "blink_dy_max": 25,      # 【垂直】瞬移一次最遠能跨多少層距,上下皆同。
                             # 觀測到的成功跨距,【不是夾出來的上限】——這個區別很重要,
                             # 見下面「低估的後果」。目前最大實測值 25:陰陽師265 的 x≈27
                             # 線上有 y=33/45/58 三層,單次瞬移 58→33(跨 25)直接越過 45,
                             # 上下皆同(log 的 seen 列表 [58,33,58] 每格都是一次瞬移)。
                             # (先前 24 來自 x≈76 只有兩層的地形、22 來自 y=49→27,都是
                             #  「那兩層剛好差這麼多」,不代表上限。)
                             #
                             # 【低估的後果】建圖靠這個值判斷「這個 x 上誰是最遠可達平台」。
                             # 填小了會把真正的落點平台判成「超出範圍、到不了」,於是把中間
                             # 那層誤認成最遠 → 建出一條實機走不到的邊 → 角色在兩層之間來回,
                             # 中間層永遠到達不了(實測填 24 時 y=45 整層失聯)。
                             # navigator._blink_to_y 會在實走跨距超過這個值時自動放寬並提示,
                             # 看到提示就把這裡一起改大。
