...
def latest_ts():
    """最新影格的時間戳(time.time());還沒有影格回 0。

    【用途:判斷「有沒有新的一格」】符文疊圖的背景迴圈以高於遊戲的頻率輪詢,
    同一格會被拿到好幾次。而角速度是逐幀相減算出來的,重複幀會生出一串 0 位移,
    把真正的晃動(角速度反轉)切碎。比對這個時間戳就能只取新格。
    刻意【不】更新 _last_read_ts —— 這支只是問「新了沒」,不算真的讀影格,
    不該讓閒置看門狗誤以為有人在用。"""
    return _frame_ts
