"""定位假設驗證(修正版):訓練一個「隨機偏移窗 → 回歸偏移量」的多任務模型,
回答「分數圖峰值定不準」的問題是不是規格設計本身的錯(見 .superpowers/detect-reg.md,
前一輪窗分類器的失敗記錄見 .superpowers/detect-mvp.md)。

【不自己重寫架構/前處理】骨幹沿用 train_rune_arrow_truth.ArrowNet 的 `feat`
(3 層 conv+BN+ReLU+MaxPool,3→32→64→128,未改一行),只換輸出頭:原本單一
`Linear(128,5)` 換成分類頭 `Linear(128,5)` + 回歸頭 `Linear(128,2)` 兩個。
前處理沿用 rune_nn.preprocess,原封不動。

【資料】沿用 tools/build_detect_data.py 的 arrow_centers / load_truth_records /
split_by_time / crop_window(邊界複製撐開)/ _random_negative_center,只新增
tools/build_detect_reg_data.py 產生「隨機偏移窗 + 回歸目標」。

【增強】沿用 train_detect.py 的 augment(色相/明暗抖動 + cutout,已經拿掉平移
—— 這裡同樣不能有平移增強,位置本身就是要學的目標)。**不做 rot90 增強**:
旋轉後方向標籤要跟著轉(既有 rot_label),偏移量 (dx,dy) 也要跟著轉,規格書
明講「容易出錯就乾脆不做,正確性優先於資料量」,這裡選擇不做。

【none 樣本不計入回歸損失】離所有箭頭中心 >40px 的隨機窗沒有對應的箭頭,回歸
目標無意義,訓練時用 is_none 遮罩把它們從 SmoothL1 loss 排除,只留分類損失。

【訓練必須前景跑完,用檢查點分段】單次跑 60 epoch 實測會超過工具逾時,所以
這支腳本支援 --ckpt 存讀進度(model/optimizer/rng state),分成多次
`--epochs-this-run` 呼叫接續,直到 epoch 數到 --epochs-total 才匯出 ONNX。

只在可拋棄的 venv-train 裡跑:
    venv-train/Scripts/python.exe tools/train_detect_reg.py --epochs-this-run 20
    (重複呼叫直到印出「已匯出」)

輸出 server/rune_detect_reg.onnx —— 全新檔名,不覆蓋任何既有 .onnx。
"""
import argparse
import os
import sys
...
import build_detect_reg_data as brd        # noqa: E402  新增:偏移窗資料
...
OUT_ONNX = os.path.join(ROOT, "server", "rune_detect_reg.onnx")
DEFAULT_TRUTH = os.path.join(ROOT, "rune_dataset", "box_truth.json")
DEFAULT_CKPT = os.path.join(ROOT, "tools", "_ckpt_detect_reg.pt")  # 訓練期暫存,非交付物
