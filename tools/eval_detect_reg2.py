"""定位假設驗證(第三輪:GAP→展平)驗收 —— 與 tools/eval_detect_reg.py 完全相同的
量測方式,唯一差異是讀取 server/rune_detect_reg2.onnx(train_detect_reg2.py 的
產出),用來跟前一輪(rune_detect_reg.onnx)的數字直接對照(見 .superpowers/
detect-reg2.md)。除了 MODEL_PATH 這一行,其餘邏輯逐字複製自 eval_detect_reg.py,
不改動原檔。
...
只在有 onnxruntime 的 venv 裡跑:
    venv/Scripts/python.exe tools/eval_detect_reg2.py
    venv/Scripts/python.exe tools/eval_detect_reg2.py --min-blob 10 --truth <path>
"""
import argparse
import os
import sys
...
MODEL_PATH = os.path.join(ROOT, "server", "rune_detect_reg2.onnx")  # 唯一差異
...
def main():
    ...
    if not os.path.exists(MODEL_PATH):
        print(f"找不到 {MODEL_PATH},先跑 train_detect_reg2.py")
        return
