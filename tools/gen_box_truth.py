...
【膠囊位置本來就會變】真值顯示位置在 x 與 y 都大幅變動(x 全距 406、y 全距 252),
即使影格尺寸幾乎全同(1368x347 佔 155/157)。使用者確認:膠囊沒有出現動畫,
【位置會變是遊戲的正常行為】。所以這個分散不是標註誤差,而是真實現象 ——
反過來說它也佐證了真值可信。

由此可直接排除兩條路:
  「固定位置」定位(實測只有 20%)
  「位置先驗 + 小範圍搜尋」—— 沒有可靠的先驗中心可用
定位只能靠【畫面內容】本身,而且必須在整個搜尋範圍內真正搜。
