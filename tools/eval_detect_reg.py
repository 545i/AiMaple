"""定位假設驗收(修正版)—— 這支腳本量的是 .superpowers/detect-reg.md 要回答的
核心問題:

    「給模型一個位置隨機偏移 ±24px 的 82x81 窗,讓它回歸出『箭頭中心相對窗中心的
     偏移量』,回歸出來的箭頭中心誤差 <=6px 的涵蓋率 >=90%(未見過的資料)?」

做法:對驗證集(按 ts 排序後 20% 的檔案,與 train_detect_reg.py 用同一份切分)
每支箭頭,產生 20 個隨機偏移窗(x/y 各自 ±24px 均勻取樣,與訓練時同分布但换一批
種子,不是訓練用過的樣本),用 server/rune_detect_reg.onnx 回歸出偏移量、換算成
預測中心,與真值中心的 Chebyshev 距離(x/y 誤差取最大值)就是這支箭頭這次抽樣的
誤差。

只在有 onnxruntime 的 venv 裡跑:
    venv/Scripts/python.exe tools/eval_detect_reg.py
    venv/Scripts/python.exe tools/eval_detect_reg.py --min-blob 10 --truth <path>
"""
import argparse
import os
import sys
...
import build_detect_reg_data as brd        # noqa: E402
import rune_cv                              # noqa: E402
import rune_nn                              # noqa: E402

MODEL_PATH = os.path.join(ROOT, "server", "rune_detect_reg.onnx")
...
def main():
    ...
    if not os.path.exists(MODEL_PATH):
        print(f"找不到 {MODEL_PATH},先跑 train_detect_reg.py")
        return
    ...
    print(f"\n共 {n_arrows} 支箭頭 x {N_SAMPLES_PER_ARROW} 次抽樣 = "
          f"{len(results)} 筆(驗證集,{len(val_recs)} 張圖)\n")
